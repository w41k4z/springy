...
javac -parameters -classpath ./lib/servlet-api.jar:./lib/r-w-x_file.jar -d ./bin/framework --enable-preview --release 20 $(find  ./framework/src/ -name '*.java')
cd ./bin/framework
...
javac -parameters -classpath ./lib/servlet-api.jar:./springy-1.0.0.jar -d ./temp/WEB-INF/classes --enable-preview --release 20 $(find ./test-framework/ -name '*.java')
cd ./temp
